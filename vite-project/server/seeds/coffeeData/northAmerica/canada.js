
module.exports = {
    country: "Canada",
    coffeeHistory: {
        discovery: "Coffee became popular in Canada in the 18th century, introduced through European trade and influenced by British and French settlers. \
        The nation`s coffee culture grew alongside its development, with coffeehouses emerging as important social spaces in urban centers during the 19th and 20th centuries.",
        development: "Canada does not produce coffee beans due to its climate, but it has developed a strong coffee culture rooted in both traditional coffeehouses and modern specialty cafes. \
        Iconic chains like Tim Hortons have become synonymous with Canadian coffee culture, making coffee an everyday staple."
    },
    culturalSignificance: {
        traditions: "Coffee is a daily ritual for most Canadians, often enjoyed during breakfast or as a midday pick-me-up. \
        The phrase 'double-double,' referring to coffee with two creams and two sugars, has become a hallmark of Canadian coffee culture.",
        modern: "Canada`s urban centers have embraced the specialty coffee movement, with an increasing focus on single-origin beans, innovative brewing techniques, and artisanal coffee roasters."
    },
    regionalPreparations: {
        traditional: [
            {
                name: "Double-Double",
                description: "A Canadian classic, referring to coffee with two creams and two sugars, popularized by Tim Hortons."
            },
            {
                name: "Iced Cappuccino",
                description: "A frozen, blended coffee drink made with cream, introduced by Tim Hortons and a favorite during the summer."
            },
            {
                name: "Maple Coffee",
                description: "A coffee drink sweetened with maple syrup, reflecting Canada`s culinary heritage."
            }
        ]
    },
    interestingFacts: {
        traditions: [
            "Tim Hortons, Canada`s largest coffee chain, serves over 5 million cups of coffee daily, making it a cultural icon.",
            "Canadians rank among the top coffee consumers globally, drinking an average of 3.2 cups per day.",
            "Coffee is often paired with snacks like donuts, muffins, or bagels, reinforcing its role in social and workplace settings."
        ],
    },
    }