
const indiaData = {
    country: "India",
    output: {
        percentage: "India is the seventh-largest coffee producer in the world, accounting for around 3% of global coffee production."
    },
    coffeeHistory: {
        discovery: "Coffee was introduced to India in the 17th century by Baba Budan, a Sufi saint, who is said to have smuggled seven coffee seeds from Yemen during his pilgrimage to Mecca. \
        He planted these seeds in the Chandragiri Hills of Karnataka, marking the beginning of coffee cultivation in India.",
        development: "India became one of the first countries outside Africa and Arabia to grow coffee. \
        Under British colonial rule, coffee cultivation expanded, particularly in the southern states. \
        Today, India is known for its shade-grown coffee, cultivated under the canopy of trees, often intercropped with spices like cardamom and pepper, giving the coffee a unique flavor.",
    },
    beanProduction: {
        flavorProfiles: "Indian coffee is celebrated for its smooth, well-balanced flavor with low acidity. \
        Common flavor notes include chocolate, nutty, and spicy undertones, often attributed to the intercropping practices.",
        regions: [
            {
                region: "Karnataka: The largest coffee-producing state, known for its balanced and aromatic coffee.",
                region: "Kerala: Produces coffee with a rich body and subtle spice notes.",
                region: "Tamil Nadu: Known for its bright and fruity coffee, especially from the Nilgiris region."
            }
        ]
    },
    brewingMethods: {
        traditional: "One of the most popular ways to enjoy coffee in India is 'South Indian Filter Coffee,' made using a metal filter. \
        The coffee is brewed strong and mixed with hot milk and sugar, then served frothy in a stainless-steel tumbler and dabara (a small bowl).",
        modern: "Espresso-based drinks are growing in popularity, especially in urban cafes, where cappuccinos, lattes, and Americanos are commonly enjoyed. \
        Cold coffee, often blended with milk and sugar, is a favorite during the summer months.",
    }
    };

export default indiaData;
